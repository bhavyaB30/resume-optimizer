...
"""
    ...
    # Step 2: Skill Extraction Prompt for Missing & Common Skills
    skill_compare_prompt = PromptTemplate(
        input_variables=["jd", "resume"],
        template="""
...
3. Perform case-insensitive comparison. Ignore upper/lowercase mismatches.
4. Use these synonyms for matching:
...
{{
  ...
  "common_skills": [list of skills found in both]
}}
...
"""
    ...
    try:
        skill_chain = skill_compare_prompt | llm
        skill_response = skill_chain.invoke({"jd": data.jd, "resume": data.resume})
        if not skill_response or not skill_response.content:
            raise ValueError("Skill match LLM returned no content")

        skill_raw = skill_response.content.strip()
        if skill_raw.startswith("```json"):
            skill_raw = skill_raw[7:]
        if skill_raw.endswith("```"):
            skill_raw = skill_raw[:-3]
        skill_raw = skill_raw.strip()

        skill_data = json.loads(skill_raw)
        missing_skills = skill_data.get("missing_skills", [])
        common_skills = skill_data.get("common_skills", [])
        if not isinstance(missing_skills, list) or not isinstance(common_skills, list):
            raise ValueError("Invalid skill match format")
    except Exception as e:
        ...
        common_skills = []  # fallback does not extract common skills
